...
/*---------------------------------------------------------------------------------------------------*/	
	...
	public Boolean containsBlock(int address){
		...
		for(int k=0;k<nStreams;k++){
			...
			if(myStream[k].theseBlocks[0].isValid()){
...
/*---------------------------------------------------------------------------------------------------*/
	...
	public Block fetchBlock(int address, Cache child){
		...
		int temp_address = address&mask+ (int)(Math.pow((double)2,(double)block_bits));
		...
		int temp = address&mask;
		int lru = getLRUstream();
		System.out.println(cacheName + "-SB #"+lru+" prefetch " + temp);
		}
...
/*---------------------------------------------------------------------------------------------------*/
	...
	public void prefetchStream(int address, Cache child){
		...
		for(int k=mBlocks-1; k > 0; k--){
				...
				temp_address = temp_address&mask+ block_size;
				prefetch_count++;
	...
	public void printStreams(){
		...
		for(int k=0;k<nStreams;k++){
			for(int j=0;j<mBlocks;j++){
				...
				System.out.print((myStream[k].theseBlocks[j].getAddress()&mask));
			}
