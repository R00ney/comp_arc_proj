...
/*---------------------------------------------------------------------------------------------------*/
	...
	public Block fetchBlock(int address, Cache child){
		...
		System.out.println(cacheName + "-SB #"+kr+" prefetch " + Integer.toHexString(temp));
		}
...
/*---------------------------------------------------------------------------------------------------*/
	...
	// index is being moved from its current location to MRU
...
/*---------------------------------------------------------------------------------------------------*/
	...
	//prints out MRU stream first, and each stream from X to X+M
	public void printStreams(){
		int mask = ~(int)(Math.pow((double)2,(double)block_bits)-1);
		
		System.out.println("===== "+cacheName+"-SB contents =====");
		
		for(int k=0;k<nStreams;k++){
			if(debugValid){//Debug print statement
					int lru = myLRUorder[0+k].intValue();
					System.out.print(" #"+lru+" contains " );
			}
			for(int j=(mBlocks-1);j>=0;j--){
			
				//moves through LRU order from MRU to LRU stream
				int spot = myLRUorder[0+k].intValue();
				//moves through stream from LRU (x) to MRU (x+m)
				System.out.print(Integer.toHexString(myStream[spot].theseBlocks[j].getAddress()&mask));
				System.out.print(" ");  
			}
